...
public:
  // Konstruktor som tar emot en AbstractAvoidApiHandler, en ArrayRef och en valfri header-fil
  AvoidApiPPCallbacks(AbstractCheckApiHandler &Handler,
                      llvm::ArrayRef<llvm::StringRef> ForbiddenMacros,
                      llvm::StringRef HeaderFile = "")
      : Handler(Handler), ForbiddenMacros(ForbiddenMacros), HeaderFile(HeaderFile) {}
...
private:
  ...
  llvm::ArrayRef<llvm::StringRef> ForbiddenMacros;  // Referens till förbjudna makron
